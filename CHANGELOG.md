...
- **"AC Charge Based On" select** (register 120, EG4_HYBRID family): the setting that decides what arms grid (AC) charging — **Time** (the AC Charge schedule windows), **SOC/Volt** (the battery SOC/voltage thresholds), or **Time+SOC/Volt** (both), the vendor app's three options. Until now this selector was invisible to the integration, so a misconfiguration here silently disabled every AC Charge schedule/threshold control the integration *does* expose (a live FlexBOSS21 sat in SOC/Volt-only mode for days with its charge windows configured and inert). The related controls now also mirror the app: whichever side the firmware ignores goes **unavailable** — the AC Charge schedule time entities in SOC/Volt mode; the AC Charge SOC Limit and the AC Charge Start/End Voltage pair in Time mode — failing open (staying available) whenever the mode is unknown, so absent or unexpected data never takes working controls away. Unavailability, rather than the registry-disable treatment the SOC-vs-Voltage control regime uses, is a deliberate choice for vendor-app parity: the app hides the ignored side outright, and unlike the regime split (two parallel control sets, both always honored for their regime) the mode here makes the other side genuinely inert. The trade-off is that a gated control cannot be pre-staged until the mode is flipped. Values travel in the cloud parameter space pinned live on a FlexBOSS21 (simultaneous local-raw/cloud lockstep: raw `0x0054` alongside cloud `BIT_AC_CHARGE_TYPE = 4`): app labels Time, SOC/Volt and Time+SOC/Volt map to 0/2/4 = the raw register bits under mask `0x0E`. The implementation deliberately bypasses pylxpweb's helpers for this register on every leg — its local table models the key as single bit 3 and its cloud helpers speak a shifted 0/1/2 enum, both provably wrong against that lockstep — using a verified local read-modify-write (other register bits preserved, post-write readback must match), the portal's own `bitParamControl` for cloud writes, and a corrective overlay that re-reads the field after any locally-served parameter refresh (whose mis-decoded value would otherwise collide with the legitimate "Time" = 0). A register state outside the app's three options (possible via direct register writes — this integration's own earlier ToU automation created exactly that state) renders as an unknown selection and gates nothing.
